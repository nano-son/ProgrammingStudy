...
1. 1960년대 미국국방성에서 통신기술 연구 시작
2. 기존에는 중앙집중식 네트워크였기 때문에 공격에 대한 위험성이 컸다.
3. 분산형 네트워크 필요 >> 패킷 교환 기술을 사용
4. 미 국방성 고등연구계획국에서 패킷 교환 기술을 사용한 알파넷이라는 넷웤구축
5. 이 알파넷의 실용성 실험과 PC대 PC의 신뢰성 높은 통신을 제공하는 프로토콜 연구
6. 이게 TCP/IP이고 알파넷의 공식적인 프로토콜로 인정됨
7. 대중들에게 전파된 계기는 그 당시 널리 퍼져있던 UNIX OS에 이 TCP/IP가 탑재되었을 때 부터
...
> IP 패킷의 포트 숫자와 소스 및 목적지의 IP 주소 등을 재기록하면서 라우터를 통해 네트워크 트래픽을 주고 받는 기술을 말한다.

이 기술로 사설 네트워크에 속한 여러 개의 호스트가 하나의 공인 IP 주소를 사용하여 인터넷에 접속하는 데 사용된다.

#### 그렇다면 올때는 누가 보냈는지 어떻게 찾아오나요?

> 정답은 PAT. 네트워크 변환을 할 때 통신 포트 번호는 그대로 내버려두어서 포트번호로 서브네트워크의 호스트를 찾아간다.
